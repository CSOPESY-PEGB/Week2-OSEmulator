...
void view_process_log(const std::string& process_name) {
    ...
    if (!std::filesystem::exists(filename)) {
        std::cout << "Log file for process '" << process_name << "' not found. The process may not have started writing yet." << std::endl;
        return;
    }
    
    // Clear the main screen
    std::cout << "\x1b[2J\x1b[H";
    std::cout << "--- Viewing log for '" << process_name << "'. Type 'exit' to return. ---" << std::endl;
    ...
    while (std::cout << process_name << "> " << std::flush && std::getline(std::cin, input_line)) {
        if (input_line == "exit") {
            break;
        }
    }
