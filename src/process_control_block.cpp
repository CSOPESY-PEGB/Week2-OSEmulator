...
namespace osemu {
    ...
    PCB::PCB(std::string procName, size_t totalLines)
        : processName(std::move(procName)),
          ...
          assignedCore(std::nullopt) // Good practice to initialize optional to nullopt
    {
        // The finishTime is not initialized here, only when the process actually finishes.
    }
    ...
    // Definition for status() - This is the key one you need to implement as per the new requirements
    std::string PCB::status() const {
        ...
        if (isComplete()) {
            ...
            // The UI screenshot doesn't show a "Ready" state, only "Running" or "Finished".
            // This part of the status string is for the "Running processes" list.
            oss << "Ready (in queue)   "
                << currentInstruction << " / " << totalInstructions;
        }
